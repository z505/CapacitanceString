...
procedure AddChar(c: ansichar; buf: PCapstr);
...
function EndsChar(c: ansichar; buf: PCapStr):boolean;
...
begin
  ...
  if pansichar(buf^.data)[buf^.strlen-1] = c then result:= true;
end;
...
begin
  ...
  begin
    inc(cnt);
    if pansichar(buf^.data)[i-1] <> pansichar(s)[cnt-1] then result:= false;
  end;
end;
